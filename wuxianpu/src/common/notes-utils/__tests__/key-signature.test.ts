...
describe("noteInKeys", () => {
  ...
  it("F# is in G major", () => {
    expect(
      noteInKeys(note(NoteNameBase.F, Accidental.SHARP), KeySignature.G),
    ).toBe(true);
  });
  ...
  it("C# is NOT in C major", () => {
    expect(
      noteInKeys(note(NoteNameBase.C, Accidental.SHARP), KeySignature.C),
    ).toBe(false);
  });

  it("Bb is in F major", () => {
    expect(
      noteInKeys(note(NoteNameBase.B, Accidental.FLAT), KeySignature.F),
    ).toBe(true);
  });
  ...
  it("double sharps/flats return false (not in key)", () => {
    expect(
      noteInKeys(note(NoteNameBase.C, Accidental.DOUBLE_SHARP), KeySignature.C),
    ).toBe(false);
  });
});
